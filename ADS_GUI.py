...
def listing_ADS_files_on_table():
    global toStop
    global label_done, label_path ,label_noPath
...
def Rest_All():
    ...
    global label_path, file_entry ,file_entry2 ,file_entry3, drop , file_entry4, remove_label, progress
    ######################################|
    # rest Select folder:                #|
    if (file_entry != None):             #|
...
logo = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\ADS2.png')).resize((240, 80), Image.LANCZOS))
label_logo = ttk.Label(image=logo, background="White")
label_logo.place(x=830, y=5,anchor="n",)

logo_yarmuk = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\yarmuk.png')).resize((100, 85), Image.LANCZOS))
...
def aboutUs():
    ...
    top.title("About Us")
    top.iconbitmap(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\icon.ico'))
    top.geometry('710x550')
    top.configure(background='white')
    top.resizable(width=False, height=False)
    logo2_yarmuk = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\yarmuk.png')).resize((155, 155), Image.LANCZOS))
    label2_yarmuk = ttk.Label(top,image=logo2_yarmuk, background="White")
    label2_yarmuk.place(x=620, anchor="n")

    logo_ADS = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\ADS2.png')).resize((160,100), Image.LANCZOS))
    label_ADS = ttk.Label(top, image=logo_ADS, background="White")
    label_ADS.place(x=540, y=160)

    label = ttk.Label(top,text="Welcome to our Alternate Data Stream Detection Tool! We are a team of four students from\n"
                               "Yarmouk University - Osama Aljady, Ibrahem Aldwery, Own Banihany, and Mohammad Ababneh-\n"
                               "who are passionate about digital forensics and developing innovative tools to support investigations.\n"
                               "\n"
                               "Our project was born out of a need we observed in the digital forensics field: the ability to\n"
                               "quickly and accurately detect Alternate Data Streams (ADS) in files and determine whether they\n"
                               "are malicious or not. As digital investigators, we understand the critical importance of\n"
                               "identifying potential threats and gathering evidence to support criminal investigations.\n"
                               "\n"
                               "Our team has a diverse set of skills and expertise, including software development,\n"
                               "cybersecurity, and digital forensics. We have combined our knowledge to create a powerful tool\n"
                               "that can be used by investigators to quickly and effectively identify and analyze ADS in files.\n"
                               "\n"
                               "Our mission is to provide digital investigators with a reliable and user-friendly tool for detecting\n"
                               "ADS and determining their potential threat level. We are committed to staying up-to-date with\n"
                               "the latest developments in the digital forensics field and refining our tool to meet the evolving\n"
                               "needs of investigators.\n"
                               "\n"
                               "Thank you for choosing our Alternate Data Stream Detection Tool. We are proud to be part of\n"
                               "your investigative toolkit.\n\n"
                               "Teamwork:")
    label.configure(background="White")
    label.grid(row=1, column=1, padx=5, pady=5)

    Osama = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\Osama.JPG')).resize((155, 180), Image.LANCZOS))
    label_Osama = ttk.Label(top, image=Osama)
    label_Osama.place(x=175,y=430,anchor="e")

    label_name_Osama=ttk.Label(top,text="Osama Aljadee")
    label_name_Osama.place(x=140, y=530, anchor="e")

    Ibrahim = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\Ibrahim.jpeg')).resize((155, 180), Image.LANCZOS))
    label_Ibrahim = ttk.Label(top, image=Ibrahim)
    label_Ibrahim.place(x=350, y=430, anchor="e")

    label_name_Ibrahim = ttk.Label(top, text="Ibrahim Aldwery")
    label_name_Ibrahim.place(x=325, y=530, anchor="e")

    Own = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\Own.jpg')).resize((155, 180), Image.LANCZOS))
    label_Own = ttk.Label(top, image=Own)
    label_Own.place(x=525, y=430, anchor="e")

    label_name_Own = ttk.Label(top, text="Own Banihany")
    label_name_Own.place(x=490, y=530, anchor="e")

    Mohammad = ImageTk.PhotoImage(Image.open(resource_path('C:\\Users\\Public\\Alternate_Data_Streams\\venv\\ADS_Test_file\\img\\Mohammad.jpeg')).resize( (155, 180), Image.LANCZOS))
    label_Mohammad = ttk.Label(top, image=Mohammad)
    label_Mohammad.place(x=700, y=430, anchor="e")

    label_name_Mohammad = ttk.Label(top, text="Mohammad Ababneh")
    label_name_Mohammad.place(x=675, y=530, anchor="e")

button_About = ttk.Button(root, width=15, text='about us', command=aboutUs)
button_About.pack(side="bottom", anchor="se",pady=10,padx=10)
# ----------------------RUN--------------------------
